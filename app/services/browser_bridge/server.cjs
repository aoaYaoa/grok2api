...
async function createContextWithCookies(playwrightBrowser, sso) {
  ...
  const cookiesToAdd = mergeCookies(parseCfCookies(), parseInjectedCookies(), authCookies);
...
async function submitThroughUi(slot, payload, conversationId) {
  ...
  const sendStrategies = [
    ...
    async () => {
      const editable = (await findEditableComposer(page)) || input;
      await editable.press("Enter", { timeout: 3000 });
      return true;
    },
    async () => {
      await page.keyboard.press("Enter");
  ...
  for (const strategy of sendStrategies) {
    ...
    const isTargetRequest = (request) => {
      if (!request.url().includes(targetPattern) || request.method() !== "POST") {
        return false;
      }
      const timing = request.timing && request.timing();
      return !timing?.startTime || timing.startTime * 1000 >= sendStartedAt - 2000;
    };
    const requestPromise = page.waitForRequest(isTargetRequest, { timeout: 3000 }).catch(() => null);
    const responsePromise = page.waitForResponse(
      (resp) => isTargetRequest(resp.request()),
    ...
    try {
      ...
      if (!request) {
        log("submit: send strategy did not trigger app-chat request");
        continue;
      }
      const response = await responsePromise;
      if (!response) {
        throw new BridgeError("response_timeout", "App-chat response not captured", 504);
      }
      ...
      log(`submit: send strategy failed: ${error.message || error}`);
    }
  }

  throw new BridgeError("submit_not_triggered", "Chat submit did not trigger app-chat request", 502);
}
