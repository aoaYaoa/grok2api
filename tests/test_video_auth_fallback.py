...
class FakeTokenInfo:
    def __init__(self, token: str, *, status: TokenStatus = TokenStatus.ACTIVE, quota: int = 80):
        self.token = token
        self.status = status
        self.quota = quota
...
class FakeTokenManager:
    def __init__(self):
        ...
        self._token_infos = {
            "bad-token": FakeTokenInfo("bad-token"),
            "good-token": FakeTokenInfo("good-token"),
        }

    def get_pool_name_for_token(self, token):
        return "pool-a" if token in {"bad-token", "good-token"} else None

    def get_token_info_by_token(self, token):
        return self._token_infos.get(token)
...
class VideoAuthFallbackTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_parent_post_with_cooling_preferred_token_falls_back_to_uploaded_source_image(self):
        manager = FakeTokenManager()
        manager._token_infos["bad-token"] = FakeTokenInfo(
            "bad-token", status=TokenStatus.COOLING, quota=0
        )
        uploaded_uri = "users/demo/uploaded/cooling-fallback-source.jpg"
        ...
        with patch("app.services.grok.services.video.get_token_manager", AsyncMock(return_value=manager)), \
             ...
             patch("app.services.grok.services.video.VideoCollectProcessor", FakeCollector), \
             patch("app.services.grok.utils.upload.UploadService.upload_file", AsyncMock(return_value=("file-id", uploaded_uri))) as upload_file, \
             patch.object(VideoService, "generate_from_parent_post", AsyncMock(side_effect=AssertionError("cooling preferred token should not keep parentPost path alive"))), \
             patch.object(VideoService, "generate_from_image", AsyncMock(return_value="fake-stream")) as generate_from_image:
            result = await VideoService.completions(
                "grok-imagine-1.0-video",
                messages=[{"role": "user", "content": "video prompt"}],
                ...
                resolution="480p",
                preset="normal",
                parent_post_id="11111111-2222-3333-4444-555555555555",
                source_image_url="http://localhost:18000/v1/files/image/users/demo/generated/11111111-2222-3333-4444-555555555555/image.jpg",
                preferred_token="bad-token",
            )
    ...
    async def test_single_reference_mode_uses_reference_generation_without_name_error(self):
        manager = FakeTokenManager()
        ...
        with patch("app.services.grok.services.video.get_token_manager", AsyncMock(return_value=manager)), \
             ...
             patch("app.services.grok.services.chat.MessageExtractor.extract", return_value=("video prompt", [], [])), \
             patch("app.services.grok.services.video.VideoCollectProcessor", FakeCollector), \
             patch.object(VideoService, "generate_from_image", AsyncMock(side_effect=AssertionError("single reference mode should not use generate_from_image"))), \
             patch.object(VideoService, "generate_from_parent_post", AsyncMock(side_effect=AssertionError("single reference mode should not use parentPost path"))), \
             patch.object(VideoService, "generate_from_reference_items", AsyncMock(return_value="fake-stream")) as generate_from_reference_items:
            result = await VideoService.completions(
                "grok-imagine-1.0-video",
                messages=[{"role": "user", "content": "video prompt"}],
                stream=False,
                ...
                reference_items=[
                    {
                        "image_url": "http://localhost:18000/v1/files/image/users/demo/generated/single-ref/image.jpg",
                        "source_image_url": "http://localhost:18000/v1/files/image/users/demo/generated/single-ref/image.jpg",
        ...
        self.assertEqual(result["ok"], True)
        kwargs = generate_from_reference_items.await_args.kwargs
        self.assertEqual(kwargs["token"], "good-token")
        self.assertEqual(len(kwargs["reference_items"]), 1)
