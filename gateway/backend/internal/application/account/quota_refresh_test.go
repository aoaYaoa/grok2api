...
func TestSyncStaleConsoleQuotasScansOneConfiguredBatchPerRun(t *testing.T) {
	ctx := context.Background()
	database, err := relational.OpenSQLite(ctx, filepath.Join(t.TempDir(), "console-quota-bounded-scan.db"))
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = database.Close() })
	...
	createComplete("fresh-one", now)
	secondFreshID := createComplete("fresh-two", now)
	staleID := createComplete("stale-after-first-page", staleAt)

	adapter := &consoleQuotaSnapshotAdapter{}
	service := NewService(accounts, nil, nil, nil, provider.NewRegistry(adapter), nil, nil)
	service.UpdateAccountTaskBatchSize(2)
	succeeded, failed, nextAfterID, err := service.SyncStaleConsoleQuotas(ctx, now.Add(-6*time.Hour), 0, 1)
	if err != nil {
		t.Fatal(err)
	}
	if succeeded != 0 || failed != 0 || adapter.fullCalls.Load() != 0 || nextAfterID != secondFreshID {
		t.Fatalf("first bounded scan succeeded=%d failed=%d calls=%d cursor=%d, want cursor %d", succeeded, failed, adapter.fullCalls.Load(), nextAfterID, secondFreshID)
	}
	succeeded, failed, nextAfterID, err = service.SyncStaleConsoleQuotas(ctx, now.Add(-6*time.Hour), nextAfterID, 1)
	if err != nil {
		t.Fatal(err)
	}
	if succeeded != 1 || failed != 0 || adapter.fullCalls.Load() != 1 || nextAfterID != 0 {
		t.Fatalf("second bounded scan succeeded=%d failed=%d calls=%d cursor=%d for stale account %d", succeeded, failed, adapter.fullCalls.Load(), nextAfterID, staleID)
	}
}
