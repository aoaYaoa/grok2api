...
func TestBatchUpdateRejectsEmptyUpdatesBeforeRepository(t *testing.T) {
	repo := &batchUpdateRepository{}
	service := NewService(repo, nil, nil, nil, nil, nil, nil)

	_, err := service.BatchUpdate(context.Background(), accountdomain.ProviderBuild, []uint64{1}, UpdateInput{})
	if !errors.Is(err, ErrInvalidInput) {
		t.Fatalf("error = %v, want invalid input", err)
	}
	if len(repo.updatedIDs) != 0 {
		t.Fatal("empty update reached repository")
	}
}
