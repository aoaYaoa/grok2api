...
func (s *Service) runVideoJob(parent context.Context, job media.Job, route model.Route) {
	...
	quotaMode := s.providers.QuotaMode(route.Provider, route.UpstreamModel)
	lease, err := s.selector.AcquirePinnedForKey(ctx, route.Provider, job.AccountID, route.ID, route.UpstreamModel, quotaMode, true, accountScope)
	if err != nil {
	...
	result, err := adapter.GenerateVideo(ctx, provider.VideoRequest{
		...
		Duration: job.Seconds, AspectRatio: job.Size, Resolution: job.Quality, ReferenceURLs: referenceURLs,
