...
func (s *Service) CreateVideo(ctx context.Context, input VideoInput) (media.Job, error) {
	...
	if operation == provider.VideoOperationGenerate {
		...
		if len(input.Prompt) == 0 && !hasImage && !hasRefs && !hasRefAudio && !input.IsExtension {
	...
	job := media.Job{
		...
		Status: media.StatusQueued, Progress: 0, InputJSON: inputJSON, MetadataJSON: metadataJSON, InputImageCount: len(allRefs), CreatedAt: now, UpdatedAt: now,
	}
	reserved := false
	if priced {
...
func (s *Service) runVideoJob(parent context.Context, job media.Job, route model.Route) {
	...
	for attempt := 0; ; attempt++ {
		...
		result, err = adapter.GenerateVideo(ctx, provider.VideoRequest{
			...
			Preset: metadata.Preset, IsExtension: metadata.IsExtension, ExtendPostID: metadata.ExtendPostID,
