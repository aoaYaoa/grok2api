...
func (s *Selector) acquire(ctx context.Context, provider account.Provider, modelRouteID uint64, upstreamModel, quotaMode, affinityKey string, excluded map[uint64]bool, allowQuotaProbe bool, requestedScope clientkeydomain.AccountScope, forcedEgressNodeID uint64) (lease *accountLease, err error) {
	...
	productWindows, err := s.loadWeeklyProductWindows(ctx, values, quotaProductKnown)
	if err != nil {
		return nil, err
	}
	...
	for index, candidate := range values {
		...
		quotaWindow := routingQuotaWindow(candidate, productWindows)
		if quotaWindowExhaustedForRoute(candidate.Credential.ID, quotaWindow, quotaProduct, quotaProductKnown, quotaConsumed) {
			quotaCandidates++
			if quotaWindow.ResetAt != nil {
				earliestRetry = earlierFuture(earliestRetry, *quotaWindow.ResetAt, now)
			}
			continue
		}
	...
	for _, candidate := range values {
		...
		if inference {
			...
			if quotaWindowExhaustedForRoute(candidate.Credential.ID, quotaWindow, quotaProduct, quotaProductKnown, quotaConsumed) {
				var retryAfter time.Duration
				if quotaWindow.ResetAt != nil {
					retryAfter = retryDelay(now, *quotaWindow.ResetAt)
				}
				return nil, &SelectionUnavailableError{Reason: SelectionQuotaExhausted, RetryAfter: retryAfter}
			}
		}
...
func (s *Selector) loadWeeklyProductWindows(ctx context.Context, values []account.RoutingCandidate, productKnown bool) (map[uint64]account.QuotaWindow, error) {
	if !productKnown {
		return nil, nil
	}
	...
	for _, candidate := range values {
		...
		if window != nil && window.Mode == "weekly" && len(window.Breakdown) == 0 {
			accountIDs = append(accountIDs, candidate.Credential.ID)
		}
	}
	if len(accountIDs) == 0 {
		return nil, nil
	}
	windowsByAccount, err := s.accounts.GetQuotaWindows(ctx, accountIDs)
	if err != nil {
		return nil, err
	}
	result := make(map[uint64]account.QuotaWindow, len(accountIDs))
	for accountID, windows := range windowsByAccount {
		for _, window := range windows {
			if window.Mode == "weekly" {
				result[accountID] = window
				break
			}
		}
	}
...
func routingQuotaWindow(candidate account.RoutingCandidate, productWindows map[uint64]account.QuotaWindow) *account.QuotaWindow {
	if window, ok := productWindows[candidate.Credential.ID]; ok {
		return &window
	}
	return candidate.QuotaWindow
}

func quotaWindowExhaustedForRoute(accountID uint64, window *account.QuotaWindow, productCode int, productKnown bool, consumed map[accountQuotaConsumptionKey]int) bool {
	if window == nil {
		return false
	}
	...
	remaining := window.Remaining - consumed[accountQuotaConsumptionKey{accountID: accountID, mode: window.Mode}]
	return remaining <= 0
}
...
func (s *Selector) MarkModelQuotaExhausted(ctx context.Context, credential account.Credential, billing *account.Billing, upstreamModel string, retryAfter time.Duration) {
	...
	if upstreamModel == "" {
		s.MarkFreeQuotaExhausted(ctx, credential, 0, 0, quotaRecoveryHints{Billing: billing, RetryAfter: retryAfter})
		return
	}
...
func (s *Selector) MarkModelAccessDenied(ctx context.Context, credential account.Credential, upstreamModel string, retryAfter time.Duration) error {
	...
	// This denial belongs to one model, not to the provider credential. Refresh
	// model overlays so the rejected route observes its block while other models
	// can continue to assemble candidates from the unchanged account base.
	s.ApplyInvalidation(repository.InvalidationEvent{
		Kind: repository.InvalidationAccountModelQuotaChanged, Provider: credential.Provider,
		AccountID: credential.ID, UpstreamModel: upstreamModel,
	})
	return nil
}
...
// prefer the upstream reset, Retry-After, billing period, and finally the local fallback.
func (s *Selector) MarkPaymentQuotaExhausted(ctx context.Context, credential account.Credential, hints quotaRecoveryHints) error {
	...
	nextProbeAt := s.resolveQuotaRecoveryAt(ctx, credential.ID, now, hints)
	return s.markFreeQuotaExhaustedAt(ctx, credential, 0, 0, now, nextProbeAt)
}
...
func (s *Selector) evictCandidate(provider account.Provider, accountID uint64) {
	...
	s.candidateMu.Lock()
	defer s.candidateMu.Unlock()
	// Candidate variants (for example pinned Build selection including
	// bot-flagged accounts) share layered base snapshots. Remove this account
	// from every matching base so a new variant cannot reconstruct it, while
	// preserving the provider version and other accounts' local quota deltas.
	for key, snapshot := range s.routingBases {
		if key.provider != provider {
			continue
		}
		values := make([]account.RoutingAccountBase, 0, len(snapshot.values))
		removed := false
		for _, base := range snapshot.values {
	...
	if s.routingAccountProvider[accountID] == provider {
		delete(s.routingAccountProvider, accountID)
	}
	for key, snapshot := range s.candidates {
		if key.provider != provider {
