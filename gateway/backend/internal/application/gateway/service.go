...
func (s *Service) CreateVoiceToken(ctx context.Context, clientKey clientkey.Key, input VoiceTokenInput) (VoiceTokenResult, error) {
	...
	lease, err := s.selector.Acquire(ctx, accountdomain.ProviderWeb, 0, "grok-chat-fast", "fast", "", map[uint64]bool{}, false)
	if err != nil {
...
func (s *Service) resolvePublicModelRoutes(ctx context.Context, publicModel string, allowModelAliases bool) ([]modeldomain.Route, string, error) {
	...
	if base, effort, ok := modeldomain.ParseReasoningModelAlias(publicModel); ok {
		...
		for _, route := range routes {
			if (modeldomain.SupportsReasoningAliases(route.Provider, route.PublicID) &&
				modeldomain.SupportsReasoningEffortForProvider(route.Provider, route.PublicID, effort)) ||
...
func (s *Service) createResponseAt(ctx context.Context, input Input, path string) (*Result, error) {
	...
	forwardResponse := func(lease *accountLease, credential accountdomain.Credential, billing *accountdomain.Billing) (*provider.Response, error) {
		...
		if err == nil && !input.Streaming && response != nil && response.StatusCode >= http.StatusOK && response.StatusCode < http.StatusMultipleChoices {
			response.Body, err = peekNonEmptyResponseBody(response.Body)
		}
	...
	markResponseFailureAfterSuccess := func(credential accountdomain.Credential, err error) bool {
		status, retryAfter, classified := upstreamResponseErrorHealthPenalty(err, holdCfg.IdleAccountCooldown)
		if !classified {
			return false
		}
...
attemptLoop:
	for attempt := 0; attemptPolicy.allows(attempt); attempt++ {
		...
		if err != nil {
			...
			responseFailure := markResponseFailureAfterSuccess(credential, err)
			if !responseFailure {
	...
	handleResponse:
		...
		if response.StatusCode == http.StatusUnauthorized {
			...
			if response.StatusCode == http.StatusUnauthorized {
				...
				_ = s.accounts.MarkReauthRequired(context.WithoutCancel(ctx), credential.ID, "Grok Build OAuth credential rejected after refresh")
		...
		afterTeamRateLimit:
			...
			if lease.QuotaMode != "" && response.StatusCode == http.StatusTooManyRequests {
				...
				// Explicit Free exhaustion uses a rolling 24-hour window. The account
				// billing period may be weekly and must not delay the next probe.
				...
				err := s.selector.MarkPaymentQuotaExhausted(ctx, credential, quotaRecoveryHints{
					Billing: lease.Billing, QuotaMode: lease.QuotaMode, RetryAfter: retryAfter,
				})
...
// peekNonEmptyResponseBody keeps empty/idle non-streaming responses inside the
// account retry loop while replaying the byte used to prove the body exists.
func peekNonEmptyResponseBody(body io.ReadCloser) (io.ReadCloser, error) {
	if body == nil {
	...
	return &replayReadCloser{Reader: reader, source: body}, nil
}

type responseBodyReadError struct{ cause error }

func (e *responseBodyReadError) Error() string {
	return fmt.Sprintf("read upstream response body: %v", e.cause)
}
...
func upstreamResponseErrorHealthPenalty(err error, idleCooldown time.Duration) (int, time.Duration, bool) {
	...
	default:
		...
		return 0, 0, errors.As(err, &bodyReadErr)
	}
}
