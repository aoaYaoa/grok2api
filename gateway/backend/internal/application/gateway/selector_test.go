...
func TestSelectorUsesWeeklyProductBreakdownForRequestedModel(t *testing.T) {
	...
	if _, err := selector.Acquire(ctx, account.ProviderWeb, 0, "grok-imagine-image", "fast", "", nil, false); err == nil {
		t.Fatal("image should remain blocked by exhausted Imagine weekly quota")
	}
	if _, err := selector.beginSelectionSession(ctx, account.ProviderWeb, 0, "grok-imagine-image", "fast", "", nil, false); err == nil {
		t.Fatal("selection session should remain blocked by exhausted Imagine weekly quota")
	}
}
