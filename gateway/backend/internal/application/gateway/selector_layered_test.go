...
func TestSelectorEvictCandidatePreservesOtherAccountQuotaAcrossCacheVariants(t *testing.T) {
	repo := newLayeredRepositoryFixture()
	repo.bases = []account.RoutingAccountBase{
		...
		{Credential: account.Credential{ID: 2, Provider: account.ProviderBuild, Enabled: true, AuthStatus: account.AuthStatusActive}, QuotaWindow: &account.QuotaWindow{AccountID: 2, Mode: "fast", Remaining: 1}},
	}
	repo.nextBases = []account.RoutingAccountBase{repo.bases[1]}
	repo.overlays = map[string]account.RoutingOverlaySnapshot{
		...
		"model-b": {Values: []account.RoutingAccountOverlay{
			{AccountID: 1, ModelCapabilityKnown: true, SupportsModel: true},
			{AccountID: 2, ModelCapabilityKnown: true, SupportsModel: true},
		}},
	}
	selector := NewSelector(repo, nil, nil, nil, time.Hour, time.Second, time.Minute)
	now := time.Now().UTC()
	for _, includeBotFlagged := range []bool{false, true} {
		if _, err := selector.loadCandidates(context.Background(), account.ProviderBuild, 0, "model-a", "fast", now, includeBotFlagged); err != nil {
			t.Fatal(err)
		}
	}
	selector.ConsumeQuota(account.ProviderBuild, 2, "fast", 1)
	baseVersion := selector.routingBaseVersion(account.ProviderBuild)

	selector.evictCandidate(account.ProviderBuild, 1)
	if _, err := selector.loadCandidates(context.Background(), account.ProviderBuild, 0, "model-b", "fast", now); err != nil {
		t.Fatal(err)
	}

	selector.candidateMu.Lock()
	for key, snapshot := range selector.candidates {
		if key.provider != account.ProviderBuild {
			continue
		}
		for _, candidate := range snapshot.values {
			if candidate.Credential.ID == 1 {
				selector.candidateMu.Unlock()
				t.Fatalf("evicted account remained in candidate snapshot %#v", key)
			}
		}
	}
	for key, snapshot := range selector.routingBases {
		...
		for _, base := range snapshot.values {
			if base.Credential.ID == 1 {
				selector.candidateMu.Unlock()
				t.Fatalf("evicted account remained in base snapshot %#v", key)
			}
		}
	}
	selector.candidateMu.Unlock()
	if currentVersion := selector.routingBaseVersion(account.ProviderBuild); currentVersion != baseVersion {
		t.Fatalf("provider base version changed from %#v to %#v", baseVersion, currentVersion)
	}
	baseCalls, _ := repo.callCounts("model-b")
	if baseCalls != 1 {
		t.Fatalf("single-account eviction reloaded provider base %d times", baseCalls)
	}
	consumed := selector.quotaConsumptionSnapshot(account.ProviderBuild)
	if consumed[accountQuotaConsumptionKey{accountID: 2, mode: "fast"}] != 1 {
		t.Fatalf("other account quota consumption = %#v, want 1", consumed)
	}
}
