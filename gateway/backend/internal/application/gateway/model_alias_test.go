...
func TestResolvePublicModelRoutesGatesDynamicAliasesAndPreservesCompatibility(t *testing.T) {
	...
	webRoute := modeldomain.Route{
		ID: 3, PublicID: "Web/grok-4.5", Provider: account.ProviderWeb, UpstreamModel: "grok-4.5",
		Capability: modeldomain.CapabilityChat, Enabled: true,
	}
	service := &Service{
		models: &aliasRouteResolver{
			byPublic: map[string][]modeldomain.Route{"Build/grok-4.5": {route, webRoute}},
		},
	...
	if err != nil || len(routes) != 2 || effort != "" {
	...
	if err != nil || len(routes) != 1 || routes[0].Provider != account.ProviderBuild || effort != "low" {
	...
	service.models = &aliasRouteResolver{byPublic: map[string][]modeldomain.Route{"Build/grok-4.5": {webRoute}}}
	if _, _, err := service.resolvePublicModelRoutes(context.Background(), "grok-4.5-low", true); !errors.Is(err, repository.ErrNotFound) {
		t.Fatalf("Web-only reasoning alias should not resolve, got %v", err)
	}
