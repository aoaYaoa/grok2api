...
func New(ctx context.Context, cfg config.Config, logger *slog.Logger) (*Application, error) {
	...
	legacyVideoStore, err := inframedia.NewLegacyVideoStore(cfg.Legacy.CachePath)
	if err != nil {
		database.Close()
		return nil, err
	}
	...
	webAdapter.SetVideoAssetStore(legacyVideoStore)
	...
	router := httpserver.New(httpserver.Dependencies{Logger: logger, RequestTimeout: cfg.Server.RequestTimeout.Value(), MaxBodyBytes: cfg.Server.MaxBodyBytes, TrustedProxies: cfg.Server.TrustedProxies, ConcurrencyGate: inferenceConcurrency, SecureCookies: cfg.Auth.SecureCookies, SwaggerEnabled: cfg.Server.SwaggerEnabled, PublicAPIBaseURL: cfg.Frontend.EffectivePublicAPIBaseURL(), FrontendStaticPath: cfg.Frontend.StaticPath, LegacyStaticPath: cfg.Legacy.StaticPath, LegacyCachePath: cfg.Legacy.CachePath, LegacyAssetVersion: cfg.Legacy.AssetVersion, LegacyPublicEnabled: cfg.Legacy.PublicEnabled, LegacyAdminKey: cfg.Legacy.AdminKey, LegacyPublicKey: cfg.Legacy.PublicKey, LegacyClientKey: cfg.Legacy.ClientKey, LegacyAllowNSFW: cfg.Provider.Web.AllowNSFW, Readiness: readiness, TrafficReady: startup.acceptsTraffic, AdminAuth: adminService, Accounts: accountService, AccountSync: accountSyncService, Models: modelService, ClientKeys: clientKeyService, Audits: auditService, Dashboard: dashboardService, Gateway: gatewayService, Media: mediaService, Settings: settingsService, Egress: egressService, QualityGuardStatePath: qualityGuardPath("state.json"), QualityGuardConfigPath: qualityGuardPath("runtime-config.json"), QualityGuardToken: qualityGuardToken, QualityGuardProbe: qualityGuardProbe, Updates: updateService})
