...
func TestAppendReasoningModelAliasesUsesRealSupportedLevels(t *testing.T) {
	...
	webOnly := appendReasoningModelAliases([]modelListItem{{
		ID: "grok-4.5", Provider: account.ProviderWeb, Capability: modeldomain.CapabilityChat, Created: now.Unix(),
	}})
	if len(webOnly) != 1 || webOnly[0].ID != "grok-4.5" {
		t.Fatalf("Web model must not expose reasoning aliases: %#v", webOnly)
	}
}
