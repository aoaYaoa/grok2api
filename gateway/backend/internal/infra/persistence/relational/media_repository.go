...
func (r *MediaJobRepository) ListMediaJobsByClientKey(ctx context.Context, clientKeyID uint64, offset, limit int) ([]media.Job, int64, error) {
	if offset < 0 {
		offset = 0
	}
	if limit <= 0 || limit > 1000 {
...
func (r *MediaJobRepository) ListMediaJobs(ctx context.Context, input repository.MediaJobListQuery) ([]media.Job, int64, error) {
	...
	if err := query.Select(
		"id", "client_key_id", "client_key_name", "account_name", "model", "prompt", "seconds", "size", "quality",
