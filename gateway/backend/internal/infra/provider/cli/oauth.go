...
var (
	...
	oauthSensitivePairPattern = regexp.MustCompile(`(?i)(^|[^A-Za-z0-9_])"?(access_token|refresh_token|id_token|token|authorization|cookie|secret|password|credential)"?\s*[:=]\s*"?([^&\s"'<>},]+)"?`)
	oauthEmailPattern         = regexp.MustCompile(`(?i)\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b`)
	oauthURLPattern           = regexp.MustCompile(`(?i)\bhttps?://[^\s"'<>]+`)
	oauthErrorCodePattern     = regexp.MustCompile(`^[A-Za-z0-9][A-Za-z0-9._-]{0,99}$`)
)
...
func parseOAuthErrorResponse(body []byte, status int) oauthErrorDetails {
	fallbackCode := "oauth_http_" + strconv.Itoa(status)
	result := oauthErrorDetails{Code: fallbackCode}
	var payload any
	...
	result.Code = normalizeOAuthErrorCode(firstNonEmpty(
...
func redactOAuthDiagnosticText(value string) string {
	...
	value = oauthSensitivePairPattern.ReplaceAllString(value, "$1$2=[REDACTED]")
	value = oauthEmailPattern.ReplaceAllString(value, "[REDACTED_EMAIL]")
	value = oauthURLPattern.ReplaceAllString(value, "[REDACTED_URL]")
...
func normalizeOAuthErrorCode(value, fallback string) string {
	value = strings.TrimSpace(redactOAuthDiagnosticText(value))
	if !oauthErrorCodePattern.MatchString(value) {
		return fallback
	}
	return value
}
