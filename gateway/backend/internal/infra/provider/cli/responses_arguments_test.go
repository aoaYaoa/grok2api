...
func TestResponsesIntegerArgumentsPassthroughKeepsDoneArgumentsUnchanged(t *testing.T) {
	request := []byte(`{
		"model":"public",
		"stream":true,
		"tools":[{"type":"function","name":"write","parameters":{"type":"object","properties":{"mode":{"type":"integer"}}}}]
	}`)
	_, compatibility, err := normalizeResponsesRequest(request, "grok-4.5")
	if err != nil {
		t.Fatal(err)
	...
	prefix := strings.Repeat(" ", maxBufferedFunctionArgumentsBytes-len(`{"mode":`)) + `{"mode":`
	firstPayload, _ := json.Marshal(map[string]any{"type": "response.function_call_arguments.delta", "sequence_number": 1, "item_id": "fc_1", "delta": prefix})
	if outputs, err := compatibility.rewriteStreamData("response.function_call_arguments.delta", firstPayload); err != nil || len(outputs) != 0 {
		t.Fatalf("first buffered delta: outputs=%d err=%v", len(outputs), err)
	}
	overflowPayload := []byte(`{"type":"response.function_call_arguments.delta","sequence_number":2,"item_id":"fc_1","delta":"1.0}"}`)
	if outputs, err := compatibility.rewriteStreamData("response.function_call_arguments.delta", overflowPayload); err != nil || len(outputs) != 2 {
		t.Fatalf("overflow delta: outputs=%d err=%v", len(outputs), err)
	}
	arguments := prefix + "1.0}"
	donePayload, _ := json.Marshal(map[string]any{"type": "response.function_call_arguments.done", "sequence_number": 3, "item_id": "fc_1", "arguments": arguments})
	outputs, err := compatibility.rewriteStreamData("response.function_call_arguments.done", donePayload)
	if err != nil || len(outputs) != 1 {
		t.Fatalf("done outputs=%d err=%v", len(outputs), err)
	}
	if got, _ := outputs[0].Payload["arguments"].(string); got != arguments {
		t.Fatalf("passthrough done arguments were rewritten")
	}
}
