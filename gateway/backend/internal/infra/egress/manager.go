...
func (m *Manager) AcquireCredential(ctx context.Context, scope domain.Scope, credential account.Credential) (*Lease, error) {
	identity := string(credential.Provider) + "_" + fmt.Sprintf("%d", credential.ID)
	credentialCookies := ""
	if scope != domain.ScopeBuild && strings.TrimSpace(credential.EncryptedCloudflareCookie) != "" {
		cookies, err := m.cipher.Decrypt(credential.EncryptedCloudflareCookie)
		if err != nil {
			return nil, err
		}
		credentialCookies = application.SanitizeCloudflareCookies(cookies)
	}
	if credential.AuthType == account.AuthTypeSSO && strings.TrimSpace(credential.EncryptedAccessToken) != "" {
		token, err := m.cipher.Decrypt(credential.EncryptedAccessToken)
		if err != nil {
			return nil, err
		}
	...
	lease, _, err := m.acquire(ctx, scope, fmt.Sprintf("%d", credential.ID), true, credentialCookies)
	return lease, err
}
