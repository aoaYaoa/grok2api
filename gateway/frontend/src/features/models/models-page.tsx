            ...
            {modelsQuery.isPending ? (
              ...
              <VirtualTableBody items={result?.items ?? []} colSpan={9} rowHeight={56} renderRow={(model) => {
                ...
                <TableRow className="group h-14" key={model.key} data-state={selectedRoutes > 0 ? "selected" : undefined}>
                  ...
                  <TableCell className="text-center">{model.enabledState === "enabled" ? <Badge variant="secondary" className="bg-blue-500/10 text-blue-700 dark:text-blue-300">{t("common.enabled")}</Badge> : model.enabledState === "disabled" ? <Badge variant="outline" className="text-muted-foreground">{t("common.disabled")}</Badge> : <Badge variant="outline" className="text-amber-700 dark:text-amber-300">{t("models.partiallyEnabled")}</Badge>}</TableCell>
                  <TableCell className="text-center"><ModelProvider provider={model.provider} /></TableCell>
                  <TableCell className="text-center text-xs">
                    <div title={model.supportTitle}>
                      <span className="inline-flex items-baseline gap-1 tabular-nums"><span className={cn("font-medium", model.supportedMax > 0 ? "text-blue-600 dark:text-blue-400" : "text-muted-foreground")}>{model.supportedLabel}</span><span className="text-muted-foreground">/ {model.totalLabel}</span></span>
...
const endpointCapabilityMetadata = {
  ...
  image: { icon: ImageIcon, method: "POST", path: "/v1/images/generations", color: "text-cyan-600 dark:text-cyan-400" },
