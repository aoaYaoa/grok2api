    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <button type="button" className="block min-w-0 max-w-full cursor-help text-left" aria-label={`${proxied ? t("audits.egressProxy") : t("audits.egressDirect")}: ${node}`}>
          <Badge variant="outline" className={cn("max-w-full font-normal", proxied ? "border-sky-500/35 bg-sky-500/10 text-sky-700 dark:text-sky-300" : "text-muted-foreground")}>
...
function StatusBadge({ statusCode }: { statusCode: number }) {
  ...
  if (statusCode >= 200 && statusCode < 300) {
    return <Badge variant="secondary" className={cn(compactClassName, "bg-sky-500/10 text-sky-700 dark:text-sky-300")}>{statusCode}</Badge>;
  }
