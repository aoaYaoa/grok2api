...
export function ImageGrid({ images, selected, onSelect, onOpen, onEdit }: { images: GeneratedImage[]; selected?: Set<string>; onSelect?: (id: string) => void; onOpen: (image: GeneratedImage) => void; onEdit?: (image: GeneratedImage) => void }) {
  ...
  return <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{images.map((image, index) => <article key={`${image.id}-${image.createdAt}`} className={cn("overflow-hidden rounded-md border bg-card shadow-sm", selected?.has(image.id) && "ring-2 ring-primary")}>
