...
export function ImageGrid({ images, selected, onSelect, onOpen, onEdit }: { images: GeneratedImage[]; selected?: Set<string>; onSelect?: (id: string) => void; onOpen: (image: GeneratedImage) => void; onEdit?: (image: GeneratedImage) => void }) {
  if (!images.length) return <div className="workspace-empty grid min-h-56 place-items-center text-sm text-muted-foreground">生成结果会显示在这里</div>;
  return <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{images.map((image, index) => <article key={`${image.id}-${image.createdAt}`} className={cn("overflow-hidden rounded-md border bg-card shadow-sm", selected?.has(image.id) && "ring-2 ring-inset ring-primary")}>
