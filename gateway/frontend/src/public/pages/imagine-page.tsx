...
export function ImaginePage() {
  ...
  async function openImageCache() { setImageCacheOpen(true); setCacheSelected(new Set()); setImageCacheLoading(true); try { const payload = await listCachedImages(key); setCachedImages((payload.items || []).map(cachedImage).filter((item) => Boolean(item.url))); } catch (error) { toast.error(error instanceof Error ? error.message : "读取图片缓存失败"); } finally { setImageCacheLoading(false); } }
  async function removeCachedImages() { const items = cachedImages.filter((item) => cacheSelected.has(item.id)); if (!items.length || !window.confirm(`永久删除所选 ${items.length} 张缓存图片？`)) return; try { const result = await deleteCachedImages(key, items.map(({ source, cacheKey }) => ({ source, cacheKey }))); const deleted = new Set(result.deleted_keys); setCachedImages((current) => current.filter((item) => !deleted.has(item.cacheKey))); setCacheSelected(new Set()); const message = `已删除 ${result.deleted} 张，跳过 ${result.skipped} 张，失败 ${result.failed} 张`; if (result.deleted === 0 && result.failed > 0) toast.error(message); else toast.success(message); } catch (error) { toast.error(error instanceof Error ? error.message : "删除缓存图片失败"); } }
  ...
  return <section className="workspace-page">
    ...
    <Dialog open={imageCacheOpen} onOpenChange={setImageCacheOpen}><DialogContent className="max-w-5xl"><DialogHeader><DialogTitle>缓存图片</DialogTitle></DialogHeader><div className="max-h-[70dvh] overflow-auto"><div className="sticky top-0 z-10 mb-3 flex items-center justify-between gap-2 border-b bg-background py-2"><span className="text-sm text-muted-foreground">已选 {cacheSelected.size} 张</span><Button variant="outline" disabled={!cacheSelected.size} onClick={() => void removeCachedImages()}><Trash2 className="size-4" />删除所选</Button></div>{imageCacheLoading ? <div className="workspace-empty grid min-h-44 place-items-center text-sm text-muted-foreground">正在读取缓存...</div> : cachedImages.length ? <ImageGrid images={cachedImages} selected={cacheSelected} onSelect={(id) => setCacheSelected((current) => toggleCacheSelection(current, id))} onOpen={(image) => { setImageCacheOpen(false); setActive(image); setEditPrompt(""); }} onEdit={(image) => { setImageCacheOpen(false); setActive(image); setEditPrompt(""); }} /> : <div className="workspace-empty grid min-h-44 place-items-center text-sm text-muted-foreground">暂无缓存图片</div>}</div></DialogContent></Dialog>
  </section>;
}
