...
export function ImaginePage() {
  ...
  async function removeCachedImages() { const items = cachedImages.filter((item) => cacheSelected.has(item.id)); if (!items.length || !window.confirm(`永久删除所选 ${items.length} 张缓存图片？`)) return; try { const result = await deleteCachedImages(key, items.map(({ source, cacheKey }) => ({ source, cacheKey }))); const deleted = new Set(result.deleted_keys); setCachedImages((current) => current.filter((item) => !deleted.has(item.cacheKey))); setCacheSelected(new Set()); toast.success(`已删除 ${result.deleted} 张`); } catch (error) { toast.error(error instanceof Error ? error.message : "删除缓存图片失败"); } }
